...
export default function MoviePlayer({
  ...
  const startTicker = useCallback(() => {
    ...
    tickerRef.current = setInterval(() => {
      if (document.visibilityState === "visible") {
        ...
        updateProgress(
          movie.id,
          Math.floor(activeSecsRef.current),
          totalDurationSecs,
        );
      } else {
        lastTickRef.current = Date.now();
  ...
  const playerEditing = editMode && activeEditor === "player";
  ...
  const openMediaEditor = () => {
    ...
    setActiveEditor("player");
  };

  const saveMediaChanges = () => {
    onUpdate?.({
      videoUrl: mediaDraft.videoUrl,
      thumbnail: mediaDraft.thumbnail,
      quality: mediaDraft.quality,
    });
          ...
          >
            ...
            <div className="absolute top-0 left-0 right-0 flex justify-around items-center bg-[#0a0600] px-1 py-[3px]">
              {Array.from({ length: 14 }).map((_, i) => (
                <div
                  key={i}
                  className="h-[5px] w-[8px] rounded-[1px] bg-[#fdf3d8] opacity-60"
                />
              ))}
            </div>
            {/* Perforations bottom */}

    ...
    <section className="page-container py-4 xs:py-5 sm:py-6">
      <div
        className={`overflow-hidden ${
          v ? "rounded-md" : "rounded-2xl lg:rounded-3xl"
        } ${
          v
            ? V.container
            : "border border-turquoise-100 dark:border-turquoise-900/30 bg-white dark:bg-gray-900 shadow-lg"
        }`}
      >
        {/* ── Header ── */}
        <div
          className={`flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between px-4 xs:px-5 sm:px-6 py-3 xs:py-4 ${
            v
              ? V.headerBorder
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2">
            {editMode && !activeEditor && (
              <>
                <button
                  type="button"
                  onClick={openEditor}
                  className="inline-flex items-center gap-2 rounded-lg sm:rounded-full border border-turquoise-200 bg-white px-3 py-2 text-xs xs:text-sm font-semibold text-turquoise-700 transition hover:bg-turquoise-50 dark:border-turquoise-800 dark:bg-gray-900/70 dark:text-turquoise-300 dark:hover:bg-turquoise-950/30"
                >
                  <PencilLine size={14} />
                  Edit Cast
                </button>
            ...
            <span
              className={`px-3 py-1 text-xs xs:text-sm font-bold ${
                v
        ...
        <div className={`p-4 xs:p-5 sm:p-6 ${v ? V.body : ""}`}>
          {isEditing && editMode && activeEditor === "cast" && (
            <div className="mb-4 xs:mb-5 rounded-lg border border-gray-200 bg-gray-50 p-4 xs:p-5 dark:border-gray-800 dark:bg-gray-900/50">
              <label className="text-sm font-semibold text-gray-700 dark:text-gray-300">
                ...
                <textarea
                  ...
                  className="mt-2 w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs xs:text-sm font-mono outline-none transition focus:border-turquoise-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
                />
              </label>
              ...
              <div className="mt-4 flex flex-col xs:flex-row flex-wrap gap-2">
                <button
                  type="button"
                  onClick={saveChanges}
                  className="inline-flex items-center justify-center gap-2 rounded-lg sm:rounded-full bg-turquoise-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-turquoise-500"
                >
                  <Save size={14} />
                  Save Cast
                </button>
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center rounded-lg sm:rounded-full border border-gray-200 px-4 py-2 text-sm font-semibold text-gray-600 transition hover:bg-gray-100 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800"
                >
                  Cancel
                </button>
              </div>
            ...
            {paginated.map((character, i) => {
              ...
              return (
                <button
                  ...
                  className={`group flex items-center gap-2 xs:gap-3 p-2.5 xs:p-3.5 text-left transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg active:scale-95 ${
                ...
                >
                  {/* Avatar */}
                  <div
                    className={`relative h-9 w-9 xs:h-10 xs:w-10 shrink-0 overflow-hidden transition-transform duration-200 group-hover:scale-110 ${
                      v
                        ? `rounded-sm ${V.avatarBorder} ${
                            isSelected ? "ring-2 ring-amber-700/50" : ""
                          }`
                        : `rounded-full ring-2 ${
                            isSelected ? theme.ring : "ring-transparent"
                          }`
                    }`}
                  >
                    {hasPhoto ? (
                    ...
                    <div
                      className={`absolute inset-0 flex items-center justify-center text-xs xs:text-sm font-bold text-white ${
                        v ? V.avatarBg : theme.pill.split(" ")[0]
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <span
                      className={`mt-0.5 inline-block px-2 py-0.5 text-[9px] xs:text-[10px] font-semibold ${
                        v
                          ? `rounded-sm ${V.roleTag}`
                          : `rounded-full ${theme.pill}`
                      }`}
                      style={v ? V.font : undefined}
                    >
                      {character.role}
                  ...
                  {isSelected && (
                    <div
                      className={`ml-auto flex h-5 w-5 shrink-0 items-center justify-center text-[10px] font-bold text-white ${
                        v
                          ? `rounded-sm ${V.checkmark}`
          ...
          {selectedChar && (
            <div
              className={`mt-4 xs:mt-5 overflow-hidden border-2 transition-all duration-300 ${
                v ? "rounded-sm" : "rounded-lg"
              } ${
                v
                  ? `${V.panelBorder} ${V.panelBg}`
            ...
            >
              <div className="flex flex-col xs:flex-row items-center xs:items-start gap-4 xs:gap-5 sm:gap-6 p-4 xs:p-5">
                ...
                <div className="relative shrink-0">
                  {selectedChar.photo?.trim() ? (
                    <img
                      ...
                      className={`h-24 w-24 xs:h-28 xs:w-28 sm:h-32 sm:w-32 object-cover shadow-md ${
                        v
                          ? "sepia rounded-sm ring-2 ring-amber-700/40"
                    ...
                    <div
                      className={`flex h-24 w-24 xs:h-28 xs:w-28 sm:h-32 sm:w-32 items-center justify-center text-3xl xs:text-4xl font-bold text-white shadow-md ${
                        v
                          ? `rounded-sm ${V.avatarBg}`
                          : `rounded-lg ${getTheme(selectedChar.role).pill.split(" ")[0]}`
                      }`}
                    >
                  ...
                  <span
                    className={`absolute -bottom-2 left-1/2 -translate-x-1/2 whitespace-nowrap px-2.5 py-0.5 text-[9px] xs:text-[10px] font-bold shadow ${
                      v
                        ? "rounded-sm bg-amber-800/90 text-amber-100"
                        : `rounded-full ${getTheme(selectedChar.role).pill}`
                    } border`}
                    style={v ? V.font : undefined}
                  >
                ...
                <div className="min-w-0 flex-1 text-center xs:text-left">
                    ...
                    <div className="min-w-0 flex-1">
                      <h3
                        className={`font-sans text-lg xs:text-xl sm:text-2xl font-bold leading-tight ${
                          v ? V.panelName : getTheme(selectedChar.role).text
                        }`}
                        style={v ? V.font : undefined}
                      >
                        {selectedChar.name}
                  ...
                  <p
                    className={`mt-3 text-sm leading-6 xs:leading-7 ${
                      v ? V.panelBio : "text-gray-600 dark:text-gray-300"
                    }`}
                    style={v ? V.font : undefined}
                  >
                    {selectedChar.bio ??
          ...
          {totalPages > 1 &&
            (v ? (
              <div
                className="mt-4 xs:mt-5 flex flex-wrap items-center justify-center gap-1 xs:gap-1.5"
                style={V.font}
              >
                <button
                  onClick={() => {
                  ...
                  disabled={page === 0}
                  className="flex h-8 w-8 xs:w-9 items-center justify-center border-2 border-amber-700/50 bg-[#fdf3d8] text-amber-800 transition hover:border-amber-700 hover:bg-amber-100 disabled:opacity-30 dark:border-amber-800/40 dark:bg-[#1e1508] dark:text-amber-400"
                >
                  <ChevronLeft size={14} />
                </button>
                ...
                {Array.from({ length: totalPages }).map((_, i) => (
                  <button
                    ...
                    className={`h-8 w-8 text-xs font-black border-2 transition ${
                      i === page
                        ? "border-amber-700 bg-amber-700 text-amber-100"
                        : "border-amber-700/40 bg-[#fdf3d8] text-amber-800 dark:bg-[#1e1508] dark:border-amber-800/40 dark:text-amber-400"
                    }`}
                  >
                    {i + 1}
                  </button>
                ...
                <span className="w-full text-center mt-2 xs:mt-0 xs:w-auto xs:ml-2 text-[11px] xs:text-xs text-amber-800/70 dark:text-amber-600">
              ...
              <div className="mt-4 xs:mt-5 flex flex-wrap items-center justify-center gap-2">
                <button
                  ...
                  disabled={page === 0}
                  className="rounded-lg border-2 border-turquoise-200 bg-white p-2 text-turquoise-600 transition hover:bg-turquoise-50 disabled:opacity-30 dark:border-turquoise-800 dark:bg-gray-800 dark:text-turquoise-400"
                >
                  <ChevronLeft size={16} />
                </button>
                    ...
                    <button
                      ...
                      className={`h-2 rounded-full transition ${
                        i === page
                          ...
                          : "w-2 bg-gray-300 hover:bg-turquoise-300 dark:bg-gray-700"
                      }`}
                    />
                  ))}
                </div>
                ...
                <span className="w-full xs:w-auto text-center xs:text-left text-xs xs:text-sm text-gray-500 dark:text-gray-400">
                  {page + 1} / {totalPages}
                </span>
              </div>
            ))}

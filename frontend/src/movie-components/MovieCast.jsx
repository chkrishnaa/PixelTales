...
export default function MovieCast({ movie }) {
  ...
  const handleSelect = (globalIdx) => setSelected((prev) => (prev === globalIdx ? null : globalIdx));

  return (
    <section className="page-container py-6">
      <div className={`overflow-hidden ${v ? "rounded-md" : "rounded-3xl"} ${v ? V.container : "border border-turquoise-100 dark:border-turquoise-900/30 bg-white dark:bg-gray-900 shadow-lg"}`}>

        {/* ── Header ── */}
        <div className={`flex items-center justify-between px-6 py-4 ${v ? V.headerBorder : "border-b border-turquoise-100 dark:border-turquoise-900/30 bg-linear-to-r from-turquoise-50 to-white dark:from-turquoise-950/30 dark:to-gray-900"}`}>
          <div className="flex items-center gap-2">
            <Users size={20} className={v ? V.headerIcon : "text-turquoise-600 dark:text-turquoise-400"} />
            <h2 className={`font-display text-2xl font-bold ${v ? V.headerTitle : "text-gray-900 dark:text-white"}`}
              style={v ? V.font : undefined}>
              Characters
            </h2>
          </div>
          <span className={`px-3 py-1 text-sm font-bold ${v ? `rounded-sm ${V.headerBadge}` : "rounded-full bg-turquoise-100 dark:bg-turquoise-900/40 text-turquoise-700 dark:text-turquoise-300"}`}
            style={v ? V.font : undefined}>
        ...
        <div className={`p-6 ${v ? V.body : ""}`}>
              ...
              return (
                <button
                  ...
                  className={`group flex items-center gap-3 p-3.5 text-left transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg active:scale-95 ${v ? "rounded-sm" : "rounded-2xl"} border-2 ${
                    v
                      ? isSelected ? V.btnSelected : V.btnDefault
                      : isSelected ? `${theme.border} ${theme.bg} shadow-md` : "border-gray-200 bg-gray-50 hover:border-turquoise-200 dark:border-gray-700 dark:bg-gray-800 dark:hover:border-turquoise-800"
                  }`}
                >
                  {/* Avatar */}
                  <div className={`relative h-10 w-10 shrink-0 overflow-hidden transition-transform duration-200 group-hover:scale-110 ${
                    v
                    ...
                    <div className={`absolute inset-0 flex items-center justify-center text-sm font-bold text-white ${v ? V.avatarBg : theme.pill.split(" ")[0]} ${hasPhoto ? "hidden" : "flex"}`}>
                  ...
                  <div className="min-w-0 flex-1">
                    <p className={`truncate font-bold text-sm ${v ? V.nameText : isSelected ? theme.text : "text-gray-900 dark:text-white"}`}
                      style={v ? V.font : undefined}>
                      {character.name}
                    </p>
                    <span className={`mt-0.5 inline-block px-2 py-0.5 text-[10px] font-semibold ${v ? `rounded-sm ${V.roleTag}` : `rounded-full ${theme.pill}`}`}
                      style={v ? V.font : undefined}>
                      {character.role}
                    </span>
                  </div>
                  ...
                  {isSelected && (
                    <div className={`ml-auto flex h-5 w-5 shrink-0 items-center justify-center text-[10px] font-bold text-white ${v ? `rounded-sm ${V.checkmark}` : `rounded-full ${theme.pill.split(" ")[0]}`}`}>✓</div>
                  )}
                </button>
              );
            })}
          ...
          {selectedChar && (
            <div className={`mt-5 overflow-hidden border-2 transition-all duration-300 ${v ? "rounded-sm" : "rounded-2xl"} ${v ? `${V.panelBorder} ${V.panelBg}` : `${getTheme(selectedChar.role).border} ${getTheme(selectedChar.role).bg}`}`}>
              <div className="flex items-start gap-4 p-4 sm:gap-6 sm:p-5">
                <div className="relative shrink-0">
                  {selectedChar.photo?.trim() ? (
                    <img src={selectedChar.photo} alt={selectedChar.name}
                    ...
                    <div className={`flex h-28 w-28 items-center justify-center text-4xl font-bold text-white shadow-md sm:h-32 sm:w-32 ${v ? `rounded-sm ${V.avatarBg}` : `rounded-xl ${getTheme(selectedChar.role).pill.split(" ")[0]}`}`}>
                      {selectedChar.name.charAt(0)}
                    </div>
                  )}
                  <span className={`absolute -bottom-2 left-1/2 -translate-x-1/2 whitespace-nowrap px-2.5 py-0.5 text-[10px] font-bold shadow ${v ? "rounded-sm bg-amber-800/90 text-amber-100" : `rounded-full ${getTheme(selectedChar.role).pill}`}`}
                    style={v ? V.font : undefined}>
                ...
                <div className="min-w-0 flex-1 pt-0.5">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <h3 className={`font-display text-xl font-bold leading-tight ${v ? V.panelName : getTheme(selectedChar.role).text}`}
                        style={v ? V.font : undefined}>
                        {selectedChar.name}
                      </h3>
                      <p className={`mt-0.5 text-xs font-semibold ${v ? V.panelSub : "text-gray-500 dark:text-gray-400"}`}
                        style={v ? V.font : undefined}>
                        Featured in&nbsp;
                        <span className={`font-bold ${v ? V.panelSubBold : "text-gray-700 dark:text-gray-200"}`}>
                          {getMovieTitle(movie)}
                        </span>
                      </p>
                    </div>
                  ...
                  <p className={`mt-3 text-sm leading-relaxed ${v ? V.panelBio : "text-gray-600 dark:text-gray-300"}`}
                    style={v ? V.font : undefined}>
                    {selectedChar.bio ??
          ...
          {totalPages > 1 && (
            v ? (
              ...
              <div className="mt-5 flex items-center justify-center gap-1.5" style={V.font}>
                <button
              ...
              <div className="mt-5 flex items-center justify-center gap-3">
                ...
                <div className="flex gap-1.5">
                  {Array.from({ length: totalPages }).map((_, i) => (
                    <button key={i} onClick={() => { setPage(i); setSelected(null); }}

...
export default function RecommendedMovies({ movies, isClassic = false }) {
  ...
  return (
    <section className="page-container py-8">
      <div
        className={`overflow-hidden shadow-xl ${v ? "rounded-md" : "rounded-3xl"} ${
          v
      ...
      >
        ...
        >
          <div className="flex items-center gap-3 text-white">
            <span
              className={`flex size-10 items-center justify-center bg-white/20 backdrop-blur-sm ${v ? "rounded-sm" : "rounded-2xl"}`}
            >
              {v ? <span className="text-lg">📽️</span> : <Film size={20} />}
            </span>
            <div>
              <h2
                className="font-sans text-2xl leading-none"
                style={v ? vFont : undefined}
              >
                {v ? "More Classics" : "More Like This"}
              </h2>
              <p
                className="mt-1 flex items-center gap-1 text-xs font-semibold text-white/80"
                style={v ? vFont : undefined}
              >
                {v ? (
                  "✦ From the same era"
                  ...
                  <>
                    <Sparkles size={12} /> Handpicked for you
                  </>
                )}
              </p>
            </div>
          ...
          <span
            className={`bg-white/20 px-3 py-1 text-sm font-bold text-white backdrop-blur-sm ${v ? "rounded-sm" : "rounded-full"}`}
            style={v ? vFont : undefined}
          >
            {all.length} Films
          </span>

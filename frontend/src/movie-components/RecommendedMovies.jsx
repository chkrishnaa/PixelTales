...
export default function RecommendedMovies({ movies, isClassic = false }) {
  ...
  return (
    <section className="page-container py-8">
      <div className={`overflow-hidden shadow-xl ${v ? "rounded-md" : "rounded-3xl"} ${
        v
          ? "border-2 border-dashed border-amber-700/50 dark:border-amber-800/40 bg-[#fdf3d8] dark:bg-[#1e1508]"
          : "border border-turquoise-100 dark:border-turquoise-900/30 bg-linear-to-b from-white to-turquoise-50/40 dark:from-gray-900 dark:to-gray-950"
      }`}>

        {/* ── Header ── */}
        <div className={`flex flex-wrap items-center justify-between gap-3 border-b px-6 py-5 ${
          v
            ? "border-amber-700/40 dark:border-amber-800/40"
            : "border-turquoise-100 dark:border-turquoise-900/30"
        }`}
          style={v ? { background: 'linear-gradient(90deg, #92400e, #b45309)' } : { background: 'linear-gradient(90deg, #0d9488, #22d3ee)' }}
        >
          <div className="flex items-center gap-3 text-white">
            <span className={`flex size-10 items-center justify-center bg-white/20 backdrop-blur-sm ${v ? "rounded-sm" : "rounded-2xl"}`}>
              {v ? <span className="text-lg">📽️</span> : <Film size={20} />}
            </span>
            <div>
              <h2 className="font-display text-2xl leading-none" style={v ? vFont : undefined}>
                {v ? "More Classics" : "More Like This"}
              </h2>
              <p className="mt-1 flex items-center gap-1 text-xs font-semibold text-white/80" style={v ? vFont : undefined}>
                {v ? "✦ From the same era" : <><Sparkles size={12} /> Handpicked for you</>}
              </p>
            </div>
          </div>
          <span className={`bg-white/20 px-3 py-1 text-sm font-bold text-white backdrop-blur-sm ${v ? "rounded-sm" : "rounded-full"}`} style={v ? vFont : undefined}>
            {all.length} Films
          </span>
        </div>

        {/* ── Grid ── */}
        <div className={`p-6 ${v ? "bg-[#fdf3d8] dark:bg-[#1e1508]" : ""}`}>
          ...
          {totalPages > 1 && (
            v ? (
              ...
              <div className="mt-7 flex items-center justify-center gap-1.5" style={vFont}>
                <button

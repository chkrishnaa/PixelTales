...
function LikersModal({ likers, onClose }) {
  return createPortal(
    <div
      className="fixed inset-0 z-200 flex items-end justify-center bg-black/50 p-3 xs:p-4 backdrop-blur-sm sm:items-center"
      onClick={onClose}
    >
      <div
        className="w-full max-w-sm overflow-hidden rounded-2xl sm:rounded-3xl bg-white shadow-2xl dark:bg-gray-900"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between border-b border-gray-100 px-4 py-3 xs:px-5 xs:py-4 dark:border-gray-800">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="rounded-full bg-rose-100 px-2 py-0.5 text-[10px] xs:text-xs font-bold text-rose-600 dark:bg-rose-900/30 dark:text-rose-400">
              {likers.length}
            </span>
          </div>
        ...
        <div className="max-h-72 xs:max-h-80 space-y-1 overflow-y-auto p-2.5 xs:p-3">
          {likers.length === 0 ? (
            <div className="py-6 text-center">
              ...
              <p className="text-xs xs:text-sm text-gray-400">No likes yet</p>
            </div>
          ) : (
            likers.map((liker, i) => (
              <div
                key={i}
                className="flex items-center gap-2 rounded-lg px-2.5 py-2 xs:px-3 xs:py-2.5 transition hover:bg-gray-50 dark:hover:bg-gray-800"
              >
                <div
                  className={`flex h-8 w-8 xs:h-9 xs:w-9 shrink-0 items-center justify-center rounded-full text-[10px] xs:text-xs font-bold text-white shadow-sm ${
                ...
                {liker.isMe && (
                  <span className="ml-auto shrink-0 rounded-full bg-turquoise-100 px-2 py-0.5 text-[9px] xs:text-[10px] font-bold text-turquoise-700 dark:bg-turquoise-900/40 dark:text-turquoise-300">
                    You
                  </span>
                )}
              </div>
...
function MentionBadge({ name, isClassic }) {
  ...
  return (
    <span className="group/mention relative mr-0.5 inline-block">
      <span
        className={`cursor-pointer text-xs xs:text-sm font-semibold hover:underline ${
          isClassic
      ...
      <div className="pointer-events-none invisible absolute bottom-full left-0 z-50 mb-2 w-52 xs:w-56 opacity-0 transition-all duration-200 group-hover/mention:visible group-hover/mention:pointer-events-auto group-hover/mention:opacity-100">
        <div className="rounded-lg border border-gray-100 bg-white p-3 shadow-xl dark:border-gray-700 dark:bg-gray-800">
          <div className="flex items-center gap-2.5">
            <div
              className={`flex h-9 w-9 xs:h-10 xs:w-10 shrink-0 items-center justify-center rounded-full text-[10px] xs:text-xs font-bold text-white shadow-sm ${avatarBg} ${
                isAdminUser
                  ? `ring-2 ${
                      isClassic ? "ring-amber-500" : "ring-turquoise-400"
                    } ring-offset-1`
                  : ""
              }`}
            >
            ...
            <div className="min-w-0">
              <p className="truncate text-xs xs:text-sm font-bold text-gray-900 dark:text-white">
                {name}
              </p>

              <p
                className={`mt-0.5 truncate text-[9px] xs:text-[10px] font-semibold ${
                  isClassic
...
function CommentThread({
  ...
  return (
    <div>
      <div
        className={`p-3 xs:p-3.5 sm:p-4 transition-all duration-200 ${
          v ? "rounded-sm" : "rounded-lg"
        } ${
          isAdmin
            ? v
              ? "border-2 border-amber-600/70 bg-[#fef3c7] dark:border-amber-600/50 dark:bg-amber-900/25 shadow-[2px_2px_0_rgba(180,83,9,0.25)] dark:shadow-[2px_2px_0_rgba(120,53,15,0.4)]"
              : "border border-turquoise-200 bg-turquoise-50/80 dark:border-turquoise-700/50 dark:bg-turquoise-950/30"
            : v
              ? "border border-amber-700/25 bg-[#f5e6c8]/50 hover:border-amber-700/60 hover:bg-amber-900/10 dark:border-amber-800/25 dark:bg-[#1a1005]/50 dark:hover:border-amber-700/50 dark:hover:bg-amber-900/25"
      ...
      >
        <div className="flex items-start gap-2 xs:gap-3">
          {/* Avatar */}
          <div
            className={`flex h-8 w-8 xs:h-9 xs:w-9 shrink-0 items-center justify-center text-[11px] xs:text-xs font-bold text-white shadow-sm ${
              v ? "rounded-sm" : "rounded-full"
            } ${avatarBg} ${
              isAdmin
                ? `ring-2 ${
                    v ? "ring-amber-500" : "ring-turquoise-400"
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mb-1 flex items-start justify-between gap-2">
              <div className="flex min-w-0 flex-wrap items-center gap-1 xs:gap-1.5">
                <span
                  className={`truncate text-xs xs:text-sm font-bold ${
                    isAdmin
                      ? v
                ...
                {isAdmin && (
                  <span
                    className={`inline-flex items-center gap-1 px-1.5 xs:px-2 py-0.5 text-[9px] xs:text-[10px] font-bold ${
                      v ? "rounded-sm" : "rounded-full"
                    } ${
                      v
                        ? "border border-amber-600/40 bg-amber-600/25 text-amber-900 shadow-sm dark:bg-amber-800/50 dark:text-amber-300"
                        : "bg-turquoise-100 text-turquoise-700 dark:bg-turquoise-900/60 dark:text-turquoise-300"
                    }`}
                    style={v ? vFont : undefined}
                  >
                    <Shield size={8} className="xs:size-[9px]" />
                    {v ? "✦ Admin" : "Admin"}
                  </span>
                )}
                ...
                <span
                  className={`text-[10px] xs:text-[11px] ${
                    v
                      ? "text-amber-700/60 dark:text-amber-700"
              ...
              {isOwnComment && (
                <div className="relative ml-1 shrink-0">
                  <button
                    ...
                    className={`flex size-6 xs:size-7 items-center justify-center transition ${
                      v ? "rounded-sm" : "rounded-full"
                    } ${
                      v
                        ? "text-amber-700/60 hover:bg-amber-700/10 hover:text-amber-800 dark:text-amber-700 dark:hover:bg-amber-800/20"
                  ...
                  {showMenu && (
                    <>
                      ...
                      <div
                        className={`absolute right-0 top-8 z-20 w-32 xs:w-36 overflow-hidden border shadow-xl ${
                          v ? "rounded-sm" : "rounded-lg"
                        } ${
                          v
                      ...
                      >
                        <button
                          onClick={enterEdit}
                          className={`flex w-full items-center gap-2 px-3 py-2.5 text-xs xs:text-sm font-semibold transition ${
                            v
                              ? "text-amber-900 hover:bg-amber-100/60 hover:text-amber-800 dark:text-amber-200 dark:hover:bg-amber-900/30"
                              : "text-gray-700 hover:bg-turquoise-50 hover:text-turquoise-700 dark:text-gray-300 dark:hover:bg-turquoise-950/40"
                          }`}
                          style={v ? vFont : undefined}
                        >
                          <Pencil size={13} />
                        ...
                        <button
                          onClick={() => {
                            onDelete(comment.id, comment._dbId);
                            setShowMenu(false);
                          }}
                          className="flex w-full items-center gap-2 px-3 py-2.5 text-xs xs:text-sm font-semibold text-red-500 transition hover:bg-red-50 dark:hover:bg-red-950/30"
                          style={v ? vFont : undefined}
                        >
                          <Trash2 size={13} />
                          Delete
            ...
            {isEditing ? (
              <div className="mt-2">
                <textarea
                  ...
                  rows={3}
                  className={`w-full resize-none border px-3 py-2 text-xs xs:text-sm outline-none transition ${
                    v ? "rounded-sm" : "rounded-lg"
                  } ${
                    v
                      ? "border-amber-700/40 bg-[#fdf3d8] text-amber-900 focus:ring-2 focus:ring-amber-600/40 dark:bg-[#1e1508] dark:text-amber-200"
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  <button
                    ...
                    className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-[11px] xs:text-xs sm:text-sm font-bold text-white transition disabled:opacity-40 ${
                      v ? "rounded-sm" : "rounded-lg"
                    } ${
                      v
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 text-[11px] xs:text-xs sm:text-sm font-semibold transition ${
                      v ? "rounded-sm" : "rounded-lg"
                    } ${
                      v
                        ? "text-amber-700/70 hover:bg-amber-100/60 dark:hover:bg-amber-900/20"
                  ...
                  <span
                    className={`w-full text-[10px] xs:w-auto xs:text-[11px] ${
                      v ? "text-amber-700/50" : "text-gray-400"
                    }`}
                    style={v ? vFont : undefined}
                  >
                    Ctrl + Enter to save
              ...
              <p
                className={`break-words text-xs xs:text-sm sm:text-[15px] leading-6 sm:leading-7 ${
                  isAdmin
                    ? v
                      ? "text-amber-800 dark:text-amber-200"
                      : "text-turquoise-800 dark:text-turquoise-100"
                    : v
              ...
              >
                {comment.replyTo && (
                  <MentionBadge name={comment.replyTo} isClassic={v} />
                )}
                {comment.text}
              </p>
            )}
            ...
            {!isEditing && (
              <div className="mt-2.5 flex flex-wrap items-center gap-x-4 gap-y-2">
                {/* Like */}
                <button
                  onClick={() => onLike(comment.id, comment._dbId)}
                  className={`inline-flex items-center gap-1.5 text-[11px] xs:text-xs sm:text-sm font-semibold transition-colors ${
                    comment.likedByMe
                      ? "text-rose-500 dark:text-rose-400"
                      : v
                        ? "text-amber-700/60 hover:text-rose-500 dark:text-amber-700 dark:hover:text-rose-400"
                ...
                <button
                  ...
                  className={`text-[11px] xs:text-xs sm:text-sm font-semibold transition-colors ${
                    showReplyInput
                      ? v
                        ? "text-amber-700 dark:text-amber-500"
                        : "text-turquoise-600 dark:text-turquoise-400"
                      : v
                        ? "text-amber-700/60 hover:text-amber-700 dark:text-amber-700 dark:hover:text-amber-500"
                ...
                {hasReplies && (
                  <button
                    ...
                    className={`inline-flex items-center gap-1 text-[11px] xs:text-xs sm:text-sm font-semibold transition-colors ${
                      v
                        ? "text-amber-700 hover:text-amber-800 dark:text-amber-500 dark:hover:text-amber-400"
                        : "text-turquoise-600 hover:text-turquoise-700 dark:text-turquoise-400"
                    }`}
                    style={v ? vFont : undefined}
                  >
                    {isOpen ? (
                ...
                {/* Likes Counter */}
                {comment.likes > 0 && (
                  <div className="group/likecount relative ml-auto sm:ml-auto">
                    <button
                      onClick={() => setShowLikers(true)}
                      className={`inline-flex items-center gap-1 text-[11px] xs:text-xs sm:text-sm font-semibold transition-colors hover:text-rose-500 dark:hover:text-rose-400 ${
                        v
                          ? "text-amber-700/60 dark:text-amber-700"
                          : "text-gray-400"
                      }`}
                    >
                      <Heart
                        size={12}
                        className={
                          comment.likedByMe ? "fill-rose-400 text-rose-400" : ""
                        }
                      />
                    ...
                    <div className="pointer-events-none invisible absolute bottom-full right-0 z-30 mb-1.5 opacity-0 transition-all duration-150 group-hover/likecount:visible group-hover/likecount:opacity-100">
                      <div className="whitespace-nowrap rounded-lg bg-gray-900 px-2 py-1 text-[10px] xs:text-[11px] font-semibold text-white shadow dark:bg-gray-700">
                        View Likes
                      </div>
                    </div>
                  </div>
        ...
        {showReplyInput && (
          <div
            className="ml-0 xs:ml-6 sm:ml-11 mt-3 flex items-start gap-2"
            onBlur={(e) => {
          ...
          >
            {/* User Badge */}
            <div
              className={`mt-1 flex h-7 w-7 xs:h-8 xs:w-8 shrink-0 items-center justify-center text-[9px] xs:text-[10px] font-bold ${
                v ? "rounded-sm" : "rounded-full"
              } ${
                v
                  ? "bg-amber-700/20 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400"
                  : "bg-turquoise-100 text-turquoise-600 dark:bg-turquoise-900/40 dark:text-turquoise-400"
              }`}
              style={v ? vFont : undefined}
            >
              You
            ...
            {/* Reply Box */}
            <div
              className={`flex flex-1 items-center gap-2 border px-2.5 xs:px-3 py-2 ${
                v ? "rounded-sm" : "rounded-full"
              } ${
                v
                  ? "border-amber-700/35 bg-[#fdf3d8] dark:border-amber-800/40 dark:bg-[#1e1508] focus-within:border-amber-600 dark:focus-within:border-amber-600"
                  : "border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900 focus-within:border-turquoise-400 dark:focus-within:border-turquoise-600"
              }`}
            >
              {/* Emoji */}
              <button
                ...
                onClick={(e) => {
                  ...
                  if (!showReplyEmoji) {
                    const rect =
                      replyEmojiBtnRef.current?.getBoundingClientRect();
                ...
                className={`shrink-0 transition ${
                  v
                    ...
                    : "text-gray-400 hover:text-turquoise-600"
                }`}
                title="Emoji"
              >
                <Smile size={15} />
              ...
              {/* Mention */}
              <span
                className={`hidden xs:block shrink-0 text-[11px] sm:text-xs font-semibold truncate max-w-24 sm:max-w-32 ${
                  v
                    ? "text-amber-700 dark:text-amber-500"
                    : "text-turquoise-600 dark:text-turquoise-400"
                }`}
              ...
              <input
                ...
                className={`flex-1 min-w-0 bg-transparent text-xs xs:text-sm outline-none placeholder:text-gray-400 ${
                  v
              ...
              <button
                ...
                className={`shrink-0 transition disabled:cursor-not-allowed disabled:opacity-30 ${
                  v
      ...
      {/* Nested replies */}
      {hasReplies && isOpen && (
        <div className="mt-2 pl-1 xs:pl-2 sm:pl-3">
          {comment.replies.map((reply, idx) => {
            ...
            return (
              <div key={reply.id} className="relative pt-2">
                ...
                {!isLast && (
                  <div
                    className={`absolute left-[12px] xs:left-[14px] sm:left-[16px] top-0 bottom-0 w-0.5 ${
                      v
                        ? "bg-amber-700/30 dark:bg-amber-800/30"
                        : "bg-turquoise-200/60 dark:bg-turquoise-800/50"
                    }`}
                  />
                )}

                {/* Vertical L Connector */}
                <div
                  className={`absolute left-[12px] xs:left-[14px] sm:left-[16px] -top-2 h-8 xs:h-8 sm:h-9 w-3 border-l-2 ${
                    v
                      ? "border-amber-700/30 dark:border-amber-800/30"
                      : "border-turquoise-200/60 dark:border-turquoise-800/50"
                  }`}
                />

                {/* Bottom Curve */}
                <div
                  className={`absolute left-[12px] xs:left-[14px] sm:left-[16px] top-6 xs:top-6 sm:top-7 h-3 w-3 rounded-bl-[8px] border-b-2 border-l-2 ${
                    v
                      ? "border-amber-700/30 dark:border-amber-800/30"
                      : "border-turquoise-200/60 dark:border-turquoise-800/50"
                  }`}
                ...
                <div className="pl-5 xs:pl-6 sm:pl-7">
                  <CommentThread
                    comment={reply}
                    depth={depth + 1}
                    parentId={comment.id}
...
export default function MovieComments({ movie }) {
  ...
  return (
    <section className="page-container py-5 xs:py-6 lg:pb-10">
      <div
      ...
      >
        {/* ── Header ── */}
        <div
          className={`flex flex-wrap items-center justify-between gap-3 px-4 py-3 xs:px-5 xs:py-4 sm:px-6 ${
            v
        ...
        >
          <div className="flex min-w-0 items-center gap-2">
            <MessageCircle
              ...
              className={
                v
                  ? "text-amber-700 dark:text-amber-500 sm:w-5 sm:h-5"
                  : "text-turquoise-600 dark:text-turquoise-400 sm:w-5 sm:h-5"
              }
            />
          ...
          <span
            className={`shrink-0 px-2.5 py-1 text-xs xs:text-sm font-bold ${
              v
                ? "rounded-sm border border-amber-700/40 bg-amber-100/60 text-amber-800 dark:bg-amber-900/20 dark:text-amber-400"
                : "rounded-full bg-turquoise-100 text-turquoise-700 dark:bg-turquoise-900/40 dark:text-turquoise-300"
            }`}
            style={v ? vFont : undefined}
          >
...
>
  ...
  <div className="mb-5">
    <div className="flex items-start gap-2 sm:gap-3">
      ...
      <div
        className={`flex flex-1 items-center gap-2 border-2 px-3 py-2 sm:px-4 transition-all ${
          v ? "rounded-sm" : "rounded-full"
        } ${
          v
      ...
      >
        ...
        <input
          ...
          className={`flex-1 min-w-0 bg-transparent text-xs xs:text-sm sm:text-base outline-none placeholder:text-gray-400 disabled:cursor-pointer ${
            v
    ...
    <div className="py-10 text-center">
      <MessageCircle
        ...
        className={`mx-auto mb-3 ${
          v
            ? "text-amber-700/40 dark:text-amber-800"
            : "text-turquoise-300 dark:text-turquoise-700"
        }`}
      />

...
import {
  ...
  Shield, MoreHorizontal, Pencil, Trash2, X, Check, Users, Loader2,
} from "lucide-react";
...
/* Recursively toggle like in the tree */
function applyLike(comments, targetId, liked, newCount) {
  return comments.map((c) => {
    if (c.id === targetId) {
      const likedBy = liked
        ? [...(c.likedBy ?? []), { name: "You", email: "" }]
        : (c.likedBy ?? []).filter((u) => u.name !== "You");
      return { ...c, likedByMe: liked, likes: newCount, likedBy };
    }
    if (c.replies?.length) return { ...c, replies: applyLike(c.replies, targetId, liked, newCount) };
    return c;
  });
}

/* Recursively soft-remove from tree */
function removeComment(comments, targetId) {
  return comments
    .filter((c) => c.id !== targetId)
    .map((c) =>
      c.replies?.length ? { ...c, replies: removeComment(c.replies, targetId) } : c
    );
}

/* Recursively edit text */
          ...
          {likers.length === 0 ? (
            ...
            likers.map((liker, i) => (
              <div key={i} className="flex items-center gap-3 rounded-xl px-3 py-2.5 transition hover:bg-gray-50 dark:hover:bg-gray-800">
                <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-xs font-bold text-white shadow-sm ${liker.name === "You" ? "bg-turquoise-500" : getAvatarColor(liker.name)}`}>
      ...
      <div className="pointer-events-none invisible absolute bottom-full left-0 z-50 mb-2 w-56 opacity-0 transition-all duration-200 group-hover/mention:visible group-hover/mention:pointer-events-auto group-hover/mention:opacity-100">
        <div className="rounded-2xl border border-gray-100 bg-white p-3 shadow-xl dark:border-gray-700 dark:bg-gray-800">
          <div className="flex items-center gap-2.5">
            <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-xs font-bold text-white shadow-sm ${avatarBg} ${isAdminUser ? "ring-2 ring-turquoise-400 ring-offset-1" : ""}`}>
              {initials}
            </div>
    ...
    <div>
      <div className={`rounded-2xl p-3.5 transition-all duration-200 ${
        ...
        <div className="flex items-start gap-3">
          {/* Avatar */}
          <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold text-white shadow-sm ${avatarBg} ${
                  ...
                  {showMenu && (
                    <>
                      ...
                      <div className="absolute right-0 top-7 z-20 w-36 overflow-hidden rounded-xl border border-gray-100 bg-white shadow-xl dark:border-gray-700 dark:bg-gray-800">
                        ...
                        <button
                          onClick={() => { onDelete(comment.id, comment._dbId); setShowMenu(false); }}
              ...
              <div className="mt-2.5 flex flex-wrap items-center gap-4">
                ...
                <button
                  ...
                  className={`text-xs font-semibold transition-colors ${
                    showReplyInput ? "text-turquoise-600 dark:text-turquoise-400" : "text-gray-400 hover:text-turquoise-600 dark:hover:text-turquoise-400"
                  }`}
                >
                  Reply
                </button>
...
export default function MovieComments({ movie }) {
    ...
    fetch(`${API}/api/movies/${movie.id}/comments`, { headers })
      ...
      .then(({ success, data }) => {
        if (!cancelled && success) {
          setComments(data ?? []);
          // Auto-open first-level replies that have content
          const ids = new Set();
  ...
  const handleLike = async (localId, dbId) => {
    // Local optimistic toggle (works even without auth — just visual)
    const wasLiked = comments.find((c) => findNode(c, localId))?.likedByMe;
    ...
    setComments((prev) => {
      ...
      return applyLike(prev, localId, liked, newCount);
    });

    if (!dbId || dbId.startsWith("local-")) return; // local-only node, no API call
    ...
    try {
      ...
      if (res.ok && data.success) {
        setComments((prev) =>
          applyLike(prev, localId, data.data.liked, data.data.likes)
        );
      }
    } catch {}
  };
  ...
  const handleReply = async (parentLocalId, parentAuthor, localReply) => {
    ...
    // Find the db ID of the parent comment
    ...
    try {
      const res  = await fetch(`${API}/api/movies/${movie.id}/comments`, {
        ...
        body: JSON.stringify({
          ...
          replyToName: parentAuthor,
        }),
      });
      const data = await res.json();
      if (res.ok && data.success) {
        // Patch the local-id node with the real dbId
        setComments((prev) =>
          patchDbId(prev, localReply.id, data.data._dbId)
        );
      }
    } catch {}
  };
          ...
          <div className="mb-5 flex items-center gap-3">
            ...
            <div className="flex flex-1 items-center gap-2 rounded-full border-2 border-gray-200 bg-gray-50 px-4 py-2 transition-all focus-within:border-turquoise-400 focus-within:bg-white dark:border-gray-700 dark:bg-gray-800/60 dark:focus-within:border-turquoise-600 dark:focus-within:bg-gray-900">
              <input
                ...
                className="flex-1 bg-transparent text-sm text-gray-700 outline-none placeholder:text-gray-400 dark:text-gray-300 disabled:cursor-not-allowed"
              />
              <button

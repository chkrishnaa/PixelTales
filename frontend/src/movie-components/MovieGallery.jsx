...
export default function MovieGallery({ movie }) {
  ...
  const goPrev = useCallback(() => setLightbox((i) => (i === 0 ? gallery.length - 1 : i - 1)), [gallery.length]);
  const goNext = useCallback(() => setLightbox((i) => (i === gallery.length - 1 ? 0 : i + 1)), [gallery.length]);
    ...
    <section className="page-container py-6">
      <div className={`overflow-hidden ${v ? "rounded-md" : "rounded-3xl"} ${v ? V.container : "border border-turquoise-100 dark:border-turquoise-900/30 bg-white dark:bg-gray-900 shadow-lg"}`}>
        ...
        <div className={`flex items-center justify-between px-6 py-4 ${v ? V.headerBorder : "border-b border-turquoise-100 dark:border-turquoise-900/30 bg-linear-to-r from-turquoise-50 to-white dark:from-turquoise-950/30 dark:to-gray-900"}`}>
          <div className="flex items-center gap-2">
            <Image size={20} className={v ? V.headerIcon : "text-turquoise-600 dark:text-turquoise-400"} />
        ...
        <div className={`p-6 ${v ? V.body : ""}`}>
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
            {paginated.map((src, i) => {
              ...
              return (
                <button key={globalIdx} onClick={() => setLightbox(globalIdx)}
                ...
                >
                  <img src={src} alt={`Screenshot ${globalIdx + 1}`}
                  ...
                  <div className={`absolute inset-0 flex items-center justify-center transition-all duration-200 ${v ? V.imgHover : "bg-turquoise-900/0 group-hover:bg-turquoise-900/50"}`}>
                    <ZoomIn size={28} className="scale-75 text-white opacity-0 drop-shadow-lg transition-all duration-200 group-hover:scale-100 group-hover:opacity-100" />
                  </div>
                  <span className={`absolute bottom-2 left-2 px-2 py-0.5 text-[10px] font-bold backdrop-blur-sm ${v ? "rounded-sm" : "rounded-lg"} ${v ? V.badge : "bg-black/60 text-white"}`}
                    style={v ? V.font : undefined}>
                    {v ? `${String(globalIdx + 1).padStart(3, '0')} / ${String(gallery.length).padStart(3, '0')}` : `${globalIdx + 1} / ${gallery.length}`}
                  </span>
                </button>
              );
            })}
      ...
      {lightbox !== null && (
        <Lightbox gallery={gallery} index={lightbox} onClose={() => setLightbox(null)} onPrev={goPrev} onNext={goNext} isClassic={v} />
      )}

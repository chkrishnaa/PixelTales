...
// Default values - will be updated when movies load
const DEFAULT_MIN_YEAR = 1980
const DEFAULT_MAX_YEAR = 2024
const DEFAULT_ALL_GENRES = []
...
export default function AllMoviesSection() {
  ...
  const ALL_GENRES = useMemo(() => {
    return Array.from(new Set(allMovies.flatMap((m) => m.genres ?? []))).sort();
  }, [allMovies]);

  const MIN_YEAR = Math.min(
    yearFrom,
    ...allMovies.map((m) => m.year).filter(Boolean),
  );
  ...
  const activeFilterCount = useMemo(
    () =>
      [
        ...
        yearFrom !== DEFAULT_MIN_YEAR || yearTo !== DEFAULT_MAX_YEAR,
        videoFilter !== "all",
        eraFilter !== "all",
        sort !== "year_desc",
      ].filter(Boolean).length,
    [query, selectedGenres, yearFrom, yearTo, videoFilter, eraFilter, sort],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return [...list].sort((a, b) => {
      switch (sort) {
        ...
        case "title_asc":
          return getAllTitles(a)[0].localeCompare(getAllTitles(b)[0]);
        case "title_desc":
          return getAllTitles(b)[0].localeCompare(getAllTitles(a)[0]);
        case "rating_desc":
  ...
  return (
    <section className="page-container pb-10">
      <SectionTitle icon={Clapperboard} action={filterButton}>
        All Movies
      </SectionTitle>

      {/* ── Result count ── */}
      <p className="-mt-3 mb-4 text-xs text-gray-400 dark:text-gray-600">
        {filtered.length} of {allMovies.length} movies
        {activeFilterCount > 0 && (
          <span className="ml-1 text-turquoise-500">(filtered)</span>
        )}
      </p>
      ...
      {panelOpen && (
        <div className="mb-6 overflow-hidden rounded-2xl border border-turquoise-200/80 bg-white shadow-xl shadow-turquoise-100/40 dark:border-turquoise-900/40 dark:bg-gray-900 dark:shadow-none">
          ...
          <div className="grid gap-6 p-5 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div>
              <label className="mb-2 block text-[11px] font-black uppercase tracking-widest text-gray-400 dark:text-gray-500">
                <ArrowUpDown size={10} className="mr-1 inline" />
                Sort By
              </label>
              <div className="flex flex-wrap gap-1.5">
                {SORT_OPTIONS.map(({ key, icon, label, sub }) => (
                  ...
                  >
                    ...
                    <span
                      className={`mt-0.5 text-[10px] ${sort === key ? "text-white/80" : "text-gray-400"}`}
                    >
                      {sub}
                    </span>
                  </button>
                ...
                <input
                  ...
                  onChange={(e) => {
                    setYearFrom(
                      Math.max(
                        MIN_YEAR,
                        Math.min(Number(e.target.value), yearTo),
                      ),
                    );
                    setCurrentPage(1);
                  }}
                ...
                <input
                  ...
                  onChange={(e) => {
                    setYearTo(
                      Math.min(
                        MAX_YEAR,
                        Math.max(Number(e.target.value), yearFrom),
                      ),
                    );
                    setCurrentPage(1);
                  }}
                ...
                {[
                  {
                    ...
                    label: "All Films",
                    icon: "🎞️",
                    cls: "border-gray-200 bg-gray-50 text-gray-600 hover:border-turquoise-300",
                  },
                  {
                    val: "modern",
                    label: "Modern",
                  ...
                  },
                  {
                    ...
                    label: "Classic",
                    icon: "📽️",
                    cls: "border-gray-200 bg-gray-50 text-gray-600 hover:border-amber-400",
                  },
            ...
            <div>
              <label className="mb-2 block text-[11px] font-black uppercase tracking-widest text-gray-400 dark:text-gray-500">
                <Tv2 size={10} className="mr-1 inline" />
                Availability
              </label>
              <div className="flex flex-wrap gap-2">
                {[
        ...
        <div className="mb-4 flex flex-wrap items-center gap-2">
          ...
          {selectedGenres.map((g) => {
            ...
            return (
              <span
                key={g}
                className={`flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-bold ${c.on}`}
              >
                {g}
                <button
                  onClick={() => toggleGenre(g)}
          ...
          {eraFilter !== "all" && (
            <span
              className={`flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-bold ${eraFilter === "classic" ? "bg-amber-500 text-white" : "bg-turquoise-600 text-white"}`}
            >
          ...
          {videoFilter !== "all" && (
            <span
              className={`flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-bold ${videoFilter === "unavailable" ? "bg-rose-500 text-white" : "bg-turquoise-600 text-white"}`}
            >
              {videoFilter === "available"
                ? "▶ Watchable only"
                : "✕ Unavailable only"}
              <button
                onClick={() => {
          ...
          {sort !== "year_desc" && (
            <span className="flex items-center gap-1.5 rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-600 dark:bg-gray-800 dark:text-gray-400">
              ...
              {SORT_OPTIONS.find((s) => s.key === sort)?.sub}
              <button
                onClick={() => {
                  setSort("year_desc");
                  setCurrentPage(1);
